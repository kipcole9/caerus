 ...
 *
 * Various mods by Kip Cole - should still be compatible though
 }}}*/
...
AutoComplete.prototype = { // {{{
  ...
  var k, def = {
    ...
    minWidth: 100,
    maxWidth: 200,
    useNotifier: false,	// Changes to be false: indicators weren't changing after Ajax returned.  Sigh.
	useIndicator: true	// Display search indicator at initialization
  };
  //}}}
  ...
  onKeyUp: function (e) { // {{{
  	...
  	if (key == Event.KEY_UP || key == Event.KEY_DOWN) 
  	{
  		...
  		Event.stop(e);
  	} else if (key == Event.KEY_ESC) {  // Esc on Safari/Mac only occurs on keyup,not keypress
	    this.clearSuggestions()
	}
  ...
  createList:	function(arr) { // {{{
	...
	// Notes by Kip Cole
	// Safari appears to calculate offset positions not
	// including any padding-top, padding-left.  So 
	// we calculate that out.  Seems to be OK so far in FF and Safari
    var pos         = this.fld.cumulativeOffset();
	var left, top;
	if (Prototype.Browser.WebKit) {
		left = pos[0] + parseFloat(this.fld.getStyle('padding-left'));
		top = pos[1] + (this.fld.offsetHeight - parseFloat(this.fld.getStyle('padding-top')));
	} else {
    	left = pos[0] + 1;  // One added because when adding bg image in FF the rendering changes.  Sigh.
    	top = pos[1] + this.fld.offsetHeight;
	};
